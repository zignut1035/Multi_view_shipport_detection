        ...
        for i in range(len(VIS_list)):
            for j in range(len(AIS_list)):
                ...
                if int(cur_mmsi) not in bin_MMSI and int(cur_ID) not in bin_ID:
                    ...
                    if dis < self.max_dis and (theta < math.pi * (1 / 3) or dis < 100):
                    ...
                    # Re-validate: does this confirmed pair still pass the
                    # same distance/heading gate a brand new candidate
                    # would need to pass? If not, its geometric agreement
                    # has broken down (e.g. it was always a marginal/wrong
                    # lock, or the real ship has since diverged from this
                    # track) -- don't keep rewarding it just for being old.
                    theta = angle(VIS_list[i], AIS_list[j])
                    x_VIS, y_VIS = VIS_list[i][-1][0], VIS_list[i][-1][1]
                    x_AIS, y_AIS = AIS_list[j][-1][0], AIS_list[j][-1][1]
                    dis = ((x_VIS - x_AIS) ** 2 + (y_VIS - y_AIS) ** 2) ** 0.5
                    ...
                    print(f"[FUS debug][{_cam_label}][CONFIRMED match={m}] VIS_ID={cur_ID} AIS_mmsi={cur_mmsi} "
                          f"dis={dis:.1f}px (max {self.max_dis}, grace {self.max_dis*2}) "
                          f"theta={_dbg_math.degrees(theta):.1f}deg (max {_dbg_math.degrees(_dbg_math.pi*1/3):.1f})")

                    if dis < self.max_dis and (theta < math.pi * (1 / 3) or dis < 100):
                        matrix_S[i][j] = -m * 100
                    else:
                        # Still allow a confirmed pair a grace margin over
                        # a fresh candidate (avoids flickering rebinds from
                        # single noisy frames), but no longer an
                        # unconditional lock -- let a much better-fitting
                        # candidate elsewhere in this tick's assignment win
                        # instead of being blocked outright.
                        matrix_S[i][j] = DTW_fast(VIS_list[i], AIS_list[j]) if (
                            dis < self.max_dis * 2 and (theta < math.pi * (1 / 3) or dis < 100)
                        ) else 1000000000
