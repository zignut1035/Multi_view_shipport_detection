...
File structure expected (produced by your bash + Python AIS scripts):
  D:/rotterdam_data/sessions/2026-05-27_15-53/
  |- cam1_KPN/
  |   |- cam1_KPN_1779864844_attempt1.mp4          (or single file .mp4, .ts, .mkv)
  |- cam2_kopvan/
  |   |- cam2_kopvan_1779864844.mp4
  |- ais/
      |- ais_20260527T135307Z_epoch1779864844.json  (+ all other poll files)

Compatibility notes vs your existing scripts:
  - bash collector  : epoch embedded in video filename -> used as video start time
  - rotterdam_AIS_tracker.py: saves record_epoch + offset_seconds in every JSON -> used
...
Usage:
  python rotterdam_matcher.py --session "D:/rotterdam_data/sessions/2026-05-27_15-53"
"""
...
# -----------------------------------------------------------------
# GATE POLYGONS  (Rotterdam Coordinates)
# -----------------------------------------------------------------
# Shared intersection zone -- a simple bounding box covering both cameras
# to quickly filter out ships that are miles away from the port.
ROTTERDAM_GATE = Polygon([
...
# Exact bounding boxes for each camera field of view
CAM_ZONES = {
    "cam1_KPN": Polygon([
        (4.476387010533255, 51.905155689368),   # SW
        (4.486652088456952, 51.90641007268441), # SE
        (4.491611063205323, 51.90867399286475), # NE
        (4.48412301119335,  51.91139666470122), # NW
    ]),
    "cam2_kopvan": Polygon([
        (4.474304241005466, 51.90044379028107),  # SW
        (4.492702037670643, 51.909163474128846), # SE
        (4.485114806161823, 51.91240615275976),  # NE
        (4.482139421256404, 51.90888814157423),  # NW
    ]),
}
...
# -----------------------------------------------------------------
# STEP 2 -- Load all AIS JSON files
# -----------------------------------------------------------------
def load_all_ais(session_dir: str) -> pd.DataFrame:
    ...
    for fpath in sorted(json_files):
        ...
        for snap in snapshots:
            ...
            record_epoch    = snap.get("record_epoch")
            offset_seconds  = snap.get("offset_seconds")
...
def sync_report(videos: dict, ais_df: pd.DataFrame, out_dir: str):
    lines = ["Rotterdam AIS <-> Video Sync Report", "=" * 50, ""]
...
# -----------------------------------------------------------------
# STEP 4 -- Filter to vessels inside the Rotterdam gate
# -----------------------------------------------------------------
def filter_in_gate(ais_df: pd.DataFrame) -> pd.DataFrame:
    mask = ais_df.apply(
        lambda r: ROTTERDAM_GATE.contains(Point(r["lon"], r["lat"]))
        if pd.notna(r["lon"]) and pd.notna(r["lat"]) else False,
        axis=1,
    )
    inside = ais_df[mask].copy()
    print(
        f"[gate] {mask.sum()} records inside Rotterdam gate "
...
def build_timeline(
    ...
    cam_zone    = CAM_ZONES.get(cam_name, ROTTERDAM_GATE)
    ...
    for offset in range(0, duration_sec + 1, interval_sec):
        ...
        for mmsi, v in latest.iterrows():
            ...
            rows.append({
                ...
                "match_status": "IN_ZONE" if in_zone else "IN_PORT",
            })
...
def print_summary(crossings_df, health_df, timelines):
    print("\n" + "=" * 60)
    print("  ROTTERDAM PORT  --  AIS Session Summary")
    print("=" * 60)
    print(f"  Vessels in port       : {len(crossings_df)}")
...
# -----------------------------------------------------------------
def main():
    ap = argparse.ArgumentParser(description="Rotterdam AIS <-> Video Matcher")
    ap.add_argument(
        "--session",
        default=r"D:\rotterdam_data\sessions\2026-05-27_15-53",
