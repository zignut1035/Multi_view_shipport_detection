...
# ===========================================================================
# DISPLAY-TIME TRACKED-AREA POLYGON
# ===========================================================================
# This restricts which AIS ships are shown/matched during video playback to
# ONLY those genuinely inside the camera's tracked river area -- separate
# from and applied AFTER interpolation (interpolation still uses the FULL
# raw AIS dataset for best track quality; this filter only affects what
# read_ais() hands back for display/fusion each frame).
#
# Order matters: these go around the shape (not skipping across it), same
# points/convention as TRACK_POLYGON in rotterdam_aisstream.py.
TRACK_POLYGON = [
    (51.91228538301762, 4.485170547106049),
    (51.909134977833325, 4.492551986154216),
    (51.906047132707876, 4.47838218074601),
    (51.904567073530096, 4.483063045606655),
]
...
def filter_to_track_polygon(df):
    """Keeps only rows whose (lat, lon) falls inside TRACK_POLYGON. Applied
    at display/matching time, not at interpolation time -- df here is
    already-interpolated data for a single frame lookup, so this is cheap
    (a handful of rows, not the whole session)."""
    if df.empty:
        return df
    mask = df.apply(lambda row: _point_in_polygon(row['lat'], row['lon'], TRACK_POLYGON), axis=1)
...
class AISPRO(object):
    def __init__(self, ais_path, ais_file, im_shape, t):
        ...
        # 3. Convert to UNIX milliseconds in a way that doesn't depend on
        #    pandas' internal datetime64 resolution (ns vs us vs ms --
        #    this varies by pandas version and silently breaks a raw
        #    `.astype('int64') // 10**6` conversion). Subtracting the
        #    epoch and taking total_seconds() is resolution-independent.
        _epoch = pd.Timestamp("1970-01-01", tz="UTC")
        self.df_all['timestamp'] = (
            (self.df_all['timestamp'] - _epoch).dt.total_seconds() * 1000
        ).astype('int64')
        # ------------------------------------------------------------
    ...
    def read_ais(self, Time_name):
        ...
        The FULL interpolated dataset (self.df_all) is used for lookup, but
        the returned frame is restricted to TRACK_POLYGON -- ships genuinely
        outside the camera's tracked river area are never shown/matched
        during display, even though they remain in the underlying CSV."""
        try:
            ...
            # Display-time restriction to the tracked river polygon --
            # applied here (after picking each ship's latest known fix),
            # not upstream at interpolation time.
            df_latest = filter_to_track_polygon(df_latest)
