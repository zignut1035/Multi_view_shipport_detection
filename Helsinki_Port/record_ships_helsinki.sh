...
python3 helsinki_AIS_tracker.py --interval $AIS_INTERVAL --duration $DURATION &
AIS_PID=$!
echo "AIS collector started (PID: $AIS_PID)"

# ── 2. Start Camera 1 (West Harbour L2 - South) ─────────────────
echo "Cam 1: Recording for 20 minutes..."
...
echo "Cam 2: Recording for 20 minutes..."
~/.local/bin/yt-dlp --remote-components ejs:github \
    --js-runtimes node \
    --downloader ffmpeg \
    --downloader-args "ffmpeg_i:-t ${DURATION}" \
...
echo "AIS data    : ./ais_data_helsinki/ais_*.json"