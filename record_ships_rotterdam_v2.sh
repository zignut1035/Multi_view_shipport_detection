...
# ── 2. Configuration ──────────────────────────────────────────────
DURATION=900         # 15 minutes total recording window
AIS_INTERVAL=30       # Kept for reference only — ignored by rotterdam_aisstream.py (live push feed, not polled)
...
pkill -INT -f "rotterdam_aisstream.py" || true
...
cleanup() {
    ...
    pkill -KILL -f "rotterdam_aisstream.py" 2>/dev/null
    sleep 2
...
# rotterdam_aisstream.py saves every qualifying update to a CSV in
# $SAVE_DIR/ais. Output is duplicated via process substitution so you see
# it live in this terminal AND it's saved to ais_log.txt. Using process
# substitution (rather than a plain pipe to tee) keeps $! pointing at the
# actual python process, so Ctrl+C / cleanup can still interrupt it directly.
echo ""
echo "--- Starting Rotterdam AIS collector (AISStream.io) ---"
timeout -s INT $DURATION python3 -u "$SCRIPT_DIR/rotterdam_aisstream.py" \
    --duration $DURATION \
    --output   "$SAVE_DIR/ais" \
    > >(tee "$SAVE_DIR/ais/ais_log.txt") 2>&1 &
AIS_PID=$!
echo "AIS PID: $AIS_PID"
echo "AIS log : $SAVE_DIR/ais/ais_log.txt"
...
record_stream() {
    ...
    while true; do
        ...
        # yt-dlp resolves %(ext)s itself once it knows the real container --
        # this prefix lets us glob for whatever file it actually creates.
        LOCAL_PREFIX="${LOCAL_OUTPUT%.%(ext)s}"
        ...
        # --- Detect the REAL recording start -----------------------------
        # RECORD_EPOCH (baked into the filename) is captured before yt-dlp
        # even launches -- it does NOT reflect when the stream actually
        # connects and the first real frame is written. Two separate delays
        # stack here: (1) local yt-dlp/ffmpeg connection+buffering time, and
        # (2) YouTube's own live-broadcast latency (the real gap between a
        # frame being captured and it becoming available to download) --
        # confirmed via #EXT-X-PROGRAM-DATE-TIME cross-referencing to be on
        # the order of several seconds to tens of seconds for these streams.
        #
        # Fix: once the local output file appears (confirming yt-dlp has
        # actually started writing), immediately query that same stream's
        # HLS manifest for its true broadcast timestamp via
        # get_stream_realtime.py (finds the LAST #EXT-X-PROGRAM-DATE-TIME
        # tag in the DVR-style manifest -- not the first, which can be
        # hours stale -- and adds the duration of segments after it to get
        # the live edge). This single number captures BOTH delays at once,
        # since it's measured at the exact real moment recording begins.
        #
        # Falls back to plain date +%s (local-only delay, no broadcast
        # delay accounted for) if the manifest fetch fails for any reason
        # -- still better than nothing, and read_all() will use whichever
        # value ends up in the .realstart sidecar either way.
        REAL_START_EPOCH=""
        for _poll in $(seq 1 150); do   # up to ~30s at 0.2s intervals
            MATCH=$(ls "${LOCAL_PREFIX}".* 2>/dev/null | head -n 1)
            if [ -n "$MATCH" ] && [ -s "$MATCH" ]; then
                MANIFEST_EPOCH=$(python3 "$SCRIPT_DIR/get_stream_realtime.py" "$URL" 2>>"$LOGFILE")
                if [ -n "$MANIFEST_EPOCH" ]; then
                    REAL_START_EPOCH="$MANIFEST_EPOCH"
                    echo "[$(date '+%H:%M:%S')] [$CAM_NAME] Real broadcast start (from HLS manifest): " \
                         "$REAL_START_EPOCH ($(date -u -d @$REAL_START_EPOCH '+%Y-%m-%d %H:%M:%S UTC'), " \
                         "file: $(basename "$MATCH")) -- $(( REAL_START_EPOCH - RECORD_EPOCH ))s after RECORD_EPOCH" \
                         | tee -a "$LOGFILE"
                else
                    REAL_START_EPOCH=$(date +%s)
                    echo "[$(date '+%H:%M:%S')] [$CAM_NAME] ⚠️  Manifest timestamp unavailable, " \
                         "falling back to local file-appearance time: $REAL_START_EPOCH " \
                         "(this only accounts for local delay, not YouTube's broadcast latency)" \
                         | tee -a "$LOGFILE"
                fi
                echo "$REAL_START_EPOCH" > "${LOCAL_PREFIX}.realstart"
                break
            fi
            sleep 0.2
        done
        if [ -z "$REAL_START_EPOCH" ]; then
            echo "[$(date '+%H:%M:%S')] [$CAM_NAME] ⚠️  Could not detect real recording start " \
                 "(no output file appeared within 30s) -- .realstart sidecar not written, " \
                 "read_all() will fall back to the filename epoch for this attempt." \
                 | tee -a "$LOGFILE"
        fi
        # -------------------------------------------------------------
