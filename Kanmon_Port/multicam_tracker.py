...
# FIX: Dropped back to 150m. We don't want ships stealing IDs 
# from old ships that were half a kilometer away.
REID_DIST_M   = 150    
...
def reidentify_new_track(cam: int, local_id: int, wx: float, wy: float, current_frame: int):
    ...
    for old_key, gid in list(_local_to_global.items()):
        ...
        # ── NEW: skip IDs that have been gone too long ────────────────────
...
def run(args):
    ...
    # Print duration info to help diagnose sync issues
    ...
    # ------------------------------------------------------------------
    # Define how long (in seconds) the tracker should remember lost ships
    # FIX: Increased to 15.0 seconds
    track_buffer_seconds = 15.0  
    
    # CAM1 Setup
    frames_buffer1 = int(fps1 * track_buffer_seconds)
    # FIX: Lowered track_thresh to 0.2
    tracker1 = ByteTrack(track_buffer=frames_buffer1, track_thresh=0.3,
                         frame_rate=int(fps1), max_age=frames_buffer1)

    # CAM2 Setup 
    frames_buffer2 = int(fps2 * track_buffer_seconds)
    # FIX: Lowered track_thresh to 0.2
    tracker2 = ByteTrack(track_buffer=frames_buffer2, track_thresh=0.3,
                         frame_rate=int(fps2), max_age=frames_buffer2)
    # ------------------------------------------------------------------
    ...
    frame_idx1 = 0
    # --- FIX: Initialize to the offset so the video text is accurate ---
    ...
    # so CAM2 output starts at the correct time
    # ------------------------------------------------------------------
    ret2, f2 = cap2.read()
    if ret2:
        frame_idx2 += 1
    ...
    # ------------------------------------------------------------------
    # 9. MAIN LOOP WITH CTRL+C HANDLING
    # ------------------------------------------------------------------
    ...
    try:
        while True:
            ret1, f1 = cap1.read()
            if not ret1:
                break  # CAM1 exhausted — stop everything
            frame_idx1 += 1

            # --- 1. Update CAM1 every loop iteration ---
            # SAFE GUARD: Catch empty detections before they hit the tracker
            ...
            if is_cam2_frame:
                ...
                if ret2:
                    ...
                    # SAFE GUARD: Catch empty detections for CAM2
                ...
                # INTERMEDIATE FRAMES: Just ensure brand new Cam1 ships get IDs
                for t1 in tc1:
                    if (1, t1["local_id"]) not in _local_to_global:
