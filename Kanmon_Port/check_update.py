"""
check_transit_updates.py
------------------------
For every vessel that crossed the camera field of view
(right edge -> left edge or vice versa), count how many
AIS updates were received during that transit.

Flags vessels with MORE THAN 2 updates while in frame.

Usage:
    python3 check_transit_updates.py --session "D:\\kanmon_data\\sessions\\2026-05-25_19-17"
    python3 check_transit_updates.py --session "/mnt/d/kanmon_data/sessions/2026-05-25_19-17"
"""
...
# Left/right edges of the gate (longitude boundaries)
GATE_WEST_LON = 130.9403   # right edge on screen (Shimonoseki side)
GATE_EAST_LON = 130.9618   # left edge on screen  (Moji side)
...
def analyse(session_dir):
    ...
    for mmsi, grp in in_gate.sort_values("time").groupby("mmsi"):
        ...
        for tid, transit in grp.groupby("transit_id"):
            ...
            # Did it reach the west edge (right side of screen)?
            touched_west = (transit["lon"] <= GATE_WEST_LON + EDGE_MARGIN).any()
            # Did it reach the east edge (left side of screen)?
...
def main():
    ...
    # ── Print results ─────────────────────────────────────────────
    print(f"\n{'='*65}")
    print(f"  AIS Update Count per Transit  |  gate threshold: >{args.min_updates}")
    print(f"{'='*65}")
